...
def placeDistortedSample(outImgTight, foregroundPixTight, BoundRect, bkgImg):
    ...
    if (outHeight >= bgHeight or outWidth >= bgWidth):
        return False, 0, 0
    ...
    boundRectFin[1][1] = float(
        BoundRect[1][0] - BoundRect[0][0]) / float(bgHeight)
    boundRectFin[1][0] = float(
        BoundRect[1][1] - BoundRect[0][1]) / float(bgWidth)
    boundRectFin[0][1] = float(posY) / float(bgHeight) + \
        boundRectFin[1][1] / float(2)
    boundRectFin[0][0] = float(posX) / float(bgWidth) + \
        boundRectFin[1][0] / float(2)
...
def main():
    ...
    for idx, sampleImgPath in enumerate(glob.glob(os.path.join(samplePath, '*.JPG'))):
        print(sampleImgPath)
        ...
        count = 0
        lower = np.array([bgColor - bgThresh, bgColor -
                         bgThresh, bgColor - bgThresh])
        upper = np.array([bgColor + bgThresh, bgColor +
                         bgThresh, bgColor + bgThresh])
        ImgModifier = SampImgModifier(
            sampleImg, dimensions, lower, upper, bgColor)

        while (count < outputPerSample):
            bkgImg = bkgFileLoader.bkgImgList[np.random.randint(
                0, bkgFileLoader.count)]
            GaussianNoiseFlag = np.less(
                np.random.uniform(0, 1), GaussianNoiseProb)
            ...
            AffineRotateFlag = np.less(
                np.random.uniform(0, 1), AffineRotateProb)
            ...
            if flag:
                ...
                cv.imwrite(os.path.join(outputfolder, str(
                    outputName + '.jpg')), finalImg)
                with open(os.path.join(outputfolder, str(outputName + '.txt')), 'w') as f:
                    details = f'{filename} ' + ' '.join(str(coord)
                                                        for coord in np.reshape(finalBoundRect, 4)) + '\n'
                    f.write(details)
                count = count + 1
            else:
                outputName = filename + '_' + str(count)
                cv.imwrite(os.path.join(outputfolder, str(
                    outputName + '.jpg')), ImgModifier.modifiedImg)
