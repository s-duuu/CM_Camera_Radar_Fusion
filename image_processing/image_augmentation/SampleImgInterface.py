...
class SampImgModifier:
    def __init__(self, image, size, lower, upper, bgcolor):
        ...
        self.image = bgcolor * \
            np.ones((self.height, self.width, self.channels), np.uint8)
        self.image[padding:(self.height-padding), padding:(self.width -
                                                           padding)] = np.copy(image[0:size[0], 0:size[1]])
        self.modifiedFlag = 0
        self.lower = lower
        self.upper = upper
        self.maskImage = self.makeMaskImage(self.image)
    ...
    def addGaussianNoise(self, noiseMean, noiseVariance):
        ...
        if(self.modifiedFlag == 1):
            ...
            self.modifiedImg[foregrndPix] = self.modifiedImg[foregrndPix] + \
                gaussImg[foregrndPix]
    ...
    def affineRotate(self, maxXangle, bgColor=255):
        ...
        if(self.modifiedFlag == 1):
            ...
            self.maskImage = self.makeMaskImage(self.modifiedImg)
        else:
            ...
            self.modifiedImg = cv.warpAffine(
                self.image, M, (nW, nH), borderMode=cv.BORDER_CONSTANT, borderValue=(bgColor, bgColor, bgColor))
            self.maskImage = self.makeMaskImage(self.modifiedImg)
            self.modifiedFlag = 1
           # cv.imshow("modified",self.modifiedImg)
           # cv.waitKey(1000)
    ...
    def perspectiveTransform(self, maxXangle, maxYangle, maxZangle, bgColor=255):
        ...
        self.modifiedImg = it.rotate_along_axis(
    ...
    def scaleImage(self, scale):
        if (self.modifiedFlag == 1):
            self.modifiedImg = cv.resize(
                self.modifiedImg, None, fx=scale, fy=scale)
            self.maskImage = self.makeMaskImage(self.modifiedImg)
    ...
    def makeMaskImage(self, image):
        ...
        copy = np.zeros(shape=np.shape(canny), dtype=np.uint8)

        cv.fillPoly(copy, cnts, 255)
        return cv.bitwise_not(copy)
    ...
    def getTightBoundbox(self):
        ...
        outImgTight = self.modifiedImg[boundRect[0][0]:(
            boundRect[1][0]+1), boundRect[0][1]:(boundRect[1][1]+1)]
